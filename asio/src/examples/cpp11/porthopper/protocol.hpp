...
class control_request
{
  public:
    ...
    {
        std::array<asio::mutable_buffer, 1> buffers = {{asio::buffer(data_)}};
        return buffers;
    }

  private:
    // Construct with specified old and new ports.
    control_request(unsigned short old_port_number, unsigned short new_port_number)
    {
...
class frame
{
  public:
    ...
    frame(unsigned long frame_number, const std::string &payload_data)
    {
        ...
        os << std::setw(payload_size) << std::setfill(' ') << payload_data.substr(0, payload_size);
    }
