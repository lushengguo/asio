...
template <void (*Test)()> inline void run_test(const char *name)
{
    test_name() = name;
...
template <void (*)()> inline void compile_test(const char *name)
{
    ASIO_TEST_IOSTREAM << name << " passed" << std::endl;
}
...
#define ASIO_CHECK(expr)                                                                                               \
    ...
    {                                                                                                                  \
        if (!(expr))                                                                                                   \
        {                                                                                                              \
            ASIO_TEST_IOSTREAM << __FILE__ << "(" << __LINE__ << "): " << asio::detail::test_name() << ": "            \
...
#define ASIO_CHECK_MESSAGE(expr, msg)                                                                                  \
    ...
    {                                                                                                                  \
        if (!(expr))                                                                                                   \
        {                                                                                                              \
            ASIO_TEST_IOSTREAM << __FILE__ << "(" << __LINE__ << "): " << asio::detail::test_name() << ": " << msg     \
                               << std::endl;                                                                           \
            ++asio::detail::test_errors();                                                                             \
        }                                                                                                              \
    } while (0)
...
#define ASIO_TEST_SUITE(name, tests)                                                                                   \
    ...
    {                                                                                                                  \
        ...
        tests return asio::detail::end_test_suite(name);                                                               \
    }

#define ASIO_TEST_CASE(test) asio::detail::run_test<&test>(#test);

#define ASIO_TEST_CASE2(test1, test2) asio::detail::run_test<&test1, test2>(#test1 "," #test2);

#define ASIO_TEST_CASE3(test1, test2, test3) asio::detail::run_test<&test1, test2, test3>(#test1 "," #test2 "," #test3);

#define ASIO_TEST_CASE4(test1, test2, test3, test4)                                                                    \
    asio::detail::run_test<&test1, test2, test3, test4>(#test1 "," #test2 "," #test3 "," #test4);

#define ASIO_TEST_CASE5(test1, test2, test3, test4, test5)                                                             \
    asio::detail::run_test<&test1, test2, test3, test4, test5>(#test1 "," #test2 "," #test3 "," #test4 "," #test5);

#define ASIO_COMPILE_TEST_CASE(test) asio::detail::compile_test<&test>(#test);
