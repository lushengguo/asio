...
template <typename Signature> struct concrete_handler;

template <typename R, typename Arg1> struct concrete_handler<R(Arg1)>
{
    concrete_handler()
    {
    }
...
template <typename R, typename Arg1, typename Arg2> struct concrete_handler<R(Arg1, Arg2)>
{
    concrete_handler()
    {
    }
...
template <typename Signature> struct immediate_concrete_handler : concrete_handler<Signature>
{
...
template <typename Signature> struct lazy_concrete_handler : concrete_handler<Signature>
{
...
template <typename Signature> class async_result<archetypes::immediate_handler, Signature>
{
  public:
    // The concrete completion handler type.
    typedef archetypes::immediate_concrete_handler<Signature> completion_handler_type;
...
template <typename Signature> class async_result<archetypes::lazy_handler, Signature>
{
  public:
    // The concrete completion handler type.
