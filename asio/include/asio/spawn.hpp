...
{
  public:
    spawned_thread_base() : owner_(0), has_context_switched_(false), throw_if_cancelled_(false), terminal_(false)
    {
    }
    ...
    template <typename F> void suspend_with(F f)
    {
  ...
  private:
    ...
    template <typename F> static void call(void *f)
    {
        (*static_cast<F *>(f))();
    }
};
...
template <typename Executor> class basic_yield_context
{
  public:
    ...
    basic_yield_context(const basic_yield_context<OtherExecutor> &other,
        ...
        : spawned_thread_(other.spawned_thread_), executor_(other.executor_), ec_(other.ec_)
    {
    }
    ...
    {
        spawned_thread_->reset_cancellation_state(static_cast<InFilter &&>(in_filter),
    ...
    basic_yield_context(detail::spawned_thread_base *spawned_thread, const Executor &ex)
        : spawned_thread_(spawned_thread), executor_(ex), ec_(0)
    {
    }
...
auto spawn(const Executor &ex, F &&function, CompletionToken &&token = default_completion_token_t<Executor>(),
#if defined(ASIO_HAS_BOOST_COROUTINE)
...
template <typename ExecutionContext, typename F,
          ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<result_of_t<
 ...
 */
template <
    typename Executor, typename StackAllocator, typename F,
    ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type)
        CompletionToken = default_completion_token_t<Executor>>
auto spawn(const Executor &ex, allocator_arg_t, StackAllocator &&stack_allocator, F &&function,
           CompletionToken &&token = default_completion_token_t<Executor>(),
           constraint_t<is_executor<Executor>::value || execution::is_executor<Executor>::value> = 0)
    -> decltype(async_initiate<CompletionToken,
                               typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type>(
...
template <typename ExecutionContext, typename StackAllocator, typename F,
          ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<result_of_t<
                                        F(basic_yield_context<typename ExecutionContext::executor_type>)>>::type)
              CompletionToken = default_completion_token_t<typename ExecutionContext::executor_type>>
auto spawn(ExecutionContext &ctx, allocator_arg_t, StackAllocator &&stack_allocator, F &&function,
           CompletionToken &&token = default_completion_token_t<typename ExecutionContext::executor_type>(),
           constraint_t<is_convertible<ExecutionContext &, execution_context &>::value> = 0)
    -> decltype(async_initiate<CompletionToken,
                               typename detail::spawn_signature<result_of_t<
...
template <
    ...
    ASIO_COMPLETION_TOKEN_FOR(typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type)
        CompletionToken = default_completion_token_t<Executor>>
auto spawn(const basic_yield_context<Executor> &ctx, allocator_arg_t, StackAllocator &&stack_allocator, F &&function,
           CompletionToken &&token = default_completion_token_t<Executor>(),
           constraint_t<is_executor<Executor>::value || execution::is_executor<Executor>::value> = 0)
    -> decltype(async_initiate<CompletionToken,
                               typename detail::spawn_signature<result_of_t<F(basic_yield_context<Executor>)>>::type>(
