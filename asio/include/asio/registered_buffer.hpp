...
class registered_buffer_id
{
  ...
  private:
    ...
    registered_buffer_id(const void *scope, int index) noexcept : scope_(scope), index_(index)
    {
    }
...
class mutable_registered_buffer
{
  ...
  private:
    ...
    mutable_registered_buffer(const mutable_buffer &b, const registered_buffer_id &i) noexcept : buffer_(b), id_(i)
    {
    }
...
class const_registered_buffer
{
  public:
    ...
    const_registered_buffer(const mutable_registered_buffer &b) noexcept : buffer_(b.buffer()), id_(b.id())
    {
    }
  ...
  private:
    ...
    const_registered_buffer(const const_buffer &b, const registered_buffer_id &i) noexcept : buffer_(b), id_(i)
    {
    }
...
inline const mutable_buffer *buffer_sequence_begin(const mutable_registered_buffer &b) noexcept
{
    return &b.buffer();
}

/// Get an iterator to the first element in a buffer sequence.
inline const const_buffer *buffer_sequence_begin(const const_registered_buffer &b) noexcept
{
    return &b.buffer();
}
...
inline mutable_registered_buffer buffer(const mutable_registered_buffer &b) noexcept
{
    return b;
}

/// Obtain a buffer representing the entire registered buffer.
inline const_registered_buffer buffer(const const_registered_buffer &b) noexcept
{
    return b;
}
 ...
 * @relates mutable_registered_buffer
 */
inline mutable_registered_buffer operator+(std::size_t n, const mutable_registered_buffer &b) noexcept
{
    return b + n;
}
 ...
 * @relates const_registered_buffer
 */
inline const_registered_buffer operator+(std::size_t n, const const_registered_buffer &b) noexcept
{
    return b + n;
}
