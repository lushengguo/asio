...
template <typename InternetProtocol> class basic_resolver_query : public resolver_query_base
{
  public:
     ...
     * may use additional locations when resolving service names.
     */
    basic_resolver_query(const protocol_type &protocol, const std::string &host, const std::string &service,
                         resolver_query_base::flags resolve_flags = address_configured)
        : hints_(), host_name_(host), service_name_(service)
    {
        hints_.ai_flags = static_cast<int>(resolve_flags);
    ...
    basic_resolver_query(const basic_resolver_query &other)
        : hints_(other.hints_), host_name_(other.host_name_), service_name_(other.service_name_)
    {
    }

    /// Move construct a @c basic_resolver_query from another.
    basic_resolver_query(basic_resolver_query &&other)
        : hints_(other.hints_), host_name_(static_cast<std::string &&>(other.host_name_)),
