...
template <typename Executor = any_io_executor> class basic_readable_pipe
{
  ...
  public:
     ...
     */
    explicit basic_readable_pipe(const executor_type &ex) : impl_(0, ex)
    {
    }
     ...
     */
    basic_readable_pipe(const executor_type &ex, const native_handle_type &native_pipe) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     */
    template <typename ExecutionContext>
    basic_readable_pipe(ExecutionContext &context, const native_handle_type &native_pipe,
     ...
     * constructor.
     */
    basic_readable_pipe(basic_readable_pipe &&other) : impl_(std::move(other.impl_))
    {
    }
  ...
  private:
    ...
    class initiate_async_read_some
    {
      public:
        ...
        explicit initiate_async_read_some(basic_readable_pipe *self) : self_(self)
        {
        }
