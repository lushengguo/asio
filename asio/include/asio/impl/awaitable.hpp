...
class awaitable_frame_base
{
  public:
    ...
    )
    {
        ...
        return awaitable_async_op<completion_signature_of_t<Op>, decay_t<Op>, Executor>
        {
...
class awaitable_frame : public awaitable_frame_base<Executor>
{
  public:
    ...
    awaitable_frame(awaitable_frame &&other) noexcept : awaitable_frame_base<Executor>(std::move(other))
    {
    }
...
{
  public:
    awaitable_frame() : top_of_stack_(0), has_executor_(false), has_context_switched_(false), throw_if_cancelled_(true)
    {
    }
  ...
  private:
    ...
    template <typename, typename>
    friend class awaitable_handler_base;
    template <typename>
    friend class awaitable_thread;
...
class awaitable_thread
{
  public:
    ...
    awaitable_thread(awaitable<awaitable_thread_entry_point, Executor> p, const Executor &ex,
    ...
    awaitable_thread(awaitable_thread &&other) noexcept : bottom_of_stack_(std::move(other.bottom_of_stack_))
    {
    }
...
class awaitable_async_op_handler<R(), Executor> : public awaitable_thread<Executor>
{
  public:
    ...
    awaitable_async_op_handler(awaitable_thread<Executor> *h, result_type &) : awaitable_thread<Executor>(std::move(*h))
    {
    }
