...
{
  public:
    promise_invoker(const shared_ptr<std::promise<T>> &p, F &&f) : p_(p), f_(static_cast<F &&>(f))
    {
    }
...
template <typename Arg> class promise_handler_selector<void(asio::error_code, Arg)> : public promise_handler_ec_1<Arg>
{
};

template <typename Arg> class promise_handler_selector<void(std::exception_ptr, Arg)> : public promise_handler_ex_1<Arg>
{
};
...
{
    explicit promise_function_wrapper(Function &f) : function_(static_cast<Function &&>(f))
    {
    }
...
{
  public:
    packaged_token(Function f, const Allocator &a) : function_(static_cast<Function &&>(f)), allocator_(a)
    {
    }
...
class packaged_handler : public promise_creator<Result>
{
  public:
    ...
    packaged_handler(packaged_token<Function, Allocator> t)
        : function_(static_cast<Function &&>(t.function_)), allocator_(t.allocator_)
    {
        this->create_promise(allocator_);
    }
    ...
    : public detail::packaged_async_result<Function, Allocator, result_of_t<Function(Args...)>>
{
  public:
    explicit async_result(typename detail::packaged_async_result<
                          Function, Allocator, result_of_t<Function(Args...)>>::completion_handler_type &h)
