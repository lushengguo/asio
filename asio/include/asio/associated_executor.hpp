...
template <typename T>
struct has_executor_type<T, void_t<typename T::executor_type>> : true_type
{
};
...
struct associated_executor_impl<T, E, void_t<typename T::executor_type>>
{
    ...
    static auto get(const T &t) noexcept -> decltype(t.get_executor())
    {
        return t.get_executor();
    }

    static auto get(const T &t, const E &) noexcept -> decltype(t.get_executor())
    {
        return t.get_executor();
    }
};
...
template <typename T, typename E>
struct associated_executor_impl<T, E, enable_if_t<!has_executor_type<T>::value>,
                                void_t<typename associator<associated_executor, T, E>::type>>
