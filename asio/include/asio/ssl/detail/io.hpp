...
class io_op : public asio::detail::base_from_cancellation_state<Handler>
{
  public:
    io_op(Stream &next_layer, stream_core &core, const Operation &op, Handler &handler)
        : asio::detail::base_from_cancellation_state<Handler>(handler), next_layer_(next_layer), core_(core), op_(op),
          start_(0), want_(engine::want_nothing), bytes_transferred_(0), handler_(static_cast<Handler &&>(handler))
    {
    }

    io_op(const io_op &other)
        : asio::detail::base_from_cancellation_state<Handler>(other), next_layer_(other.next_layer_),
          core_(other.core_), op_(other.op_), start_(other.start_), want_(other.want_), ec_(other.ec_),
    ...
    io_op(io_op &&other)
        : asio::detail::base_from_cancellation_state<Handler>(
          ...
          next_layer_(other.next_layer_), core_(other.core_), op_(static_cast<Operation &&>(other.op_)),
          start_(other.start_), want_(other.want_), ec_(other.ec_), bytes_transferred_(other.bytes_transferred_),
            ...
            do
            {
                ...
                case engine::want_output:
                    ...
                    if (core_.expiry(core_.pending_write_) == core_.neg_infin())
                    {
                        ...
                        asio::async_write(next_layer_, core_.engine_.get_output(core_.output_buffer_),
...
template <template <typename, typename> class Associator, typename Stream, typename Operation, typename Handler,
