...
class deferred_sequence_handler
{
  public:
    ...
    explicit deferred_sequence_handler(H &&handler, T &&tail)
        : handler_(static_cast<H &&>(handler)), tail_(static_cast<T &&>(tail))
    {
    }
...
class deferred_sequence_base
{
  ...
  public:
    ...
    constexpr explicit deferred_sequence_base(H &&head, T &&tail)
        : head_(static_cast<H &&>(head)), tail_(static_cast<T &&>(tail))
    {
    }
    ...
    auto operator()(CompletionToken &&token) && -> decltype(async_initiate<CompletionToken, Signatures...>(
        initiate(), token, static_cast<Head &&>(this->head_), static_cast<Tail &&>(this->tail_)))
    {
        return async_initiate<CompletionToken, Signatures...>(initiate(), token, static_cast<Head &&>(head_),
...
class deferred_function
{
  public:
    /// Constructor.
    template <typename F>
    constexpr explicit deferred_function(deferred_init_tag, F &&function) : function_(static_cast<F &&>(function))
    {
    }
...
class ASIO_NODISCARD deferred_values
{
  private:
    ...
    auto invoke_helper(CompletionToken &&token, detail::index_sequence<I...>)
        -> decltype(async_initiate<CompletionToken, void(Values...)>(
            initiate(), token, std::get<I>(static_cast<std::tuple<Values...> &&>(this->values_))...))
    {
  ...
  public:
    ...
    template <typename... V>
    constexpr explicit deferred_values(deferred_init_tag, V &&...values) : values_(static_cast<V &&>(values)...)
    {
    }
...
class ASIO_NODISCARD deferred_async_operation
{
  private:
    ...
    template <typename CompletionToken, std::size_t... I>
    auto invoke_helper(CompletionToken &&token, detail::index_sequence<I...>)
        -> decltype(async_initiate<CompletionToken, Signature>(static_cast<initiation_t &&>(initiation_), token,
                                                               std::get<I>(static_cast<init_args_t &&>(init_args_))...))
    {
        return async_initiate<CompletionToken, Signature>(static_cast<initiation_t &&>(initiation_), token,
                                                          std::get<I>(static_cast<init_args_t &&>(init_args_))...);
    }
    ...
    auto const_invoke_helper(CompletionToken &&token, detail::index_sequence<I...>)
        const & -> decltype(async_initiate<CompletionToken, Signature>(
            conditional_t<true, initiation_t, CompletionToken>(initiation_), token, std::get<I>(init_args_)...))
    {
  ...
  public:
    ...
    constexpr explicit deferred_async_operation(deferred_init_tag, I &&initiation, A &&...init_args)
        : initiation_(static_cast<I &&>(initiation)), init_args_(static_cast<A &&>(init_args)...)
    {
    }
...
class ASIO_NODISCARD deferred_async_operation<deferred_signatures<Signatures...>, Initiation, InitArgs...>
{
  private:
    ...
    {
        return async_initiate<CompletionToken, Signatures...>(static_cast<initiation_t &&>(initiation_), token,
                                                              std::get<I>(static_cast<init_args_t &&>(init_args_))...);
    }

    template <typename CompletionToken, std::size_t... I>
    auto const_invoke_helper(CompletionToken &&token, detail::index_sequence<I...>)
        const & -> decltype(async_initiate<CompletionToken, Signatures...>(initiation_t(initiation_), token,
                                                                           std::get<I>(init_args_)...))
    {
        return async_initiate<CompletionToken, Signatures...>(initiation_t(initiation_), token,
                                                              std::get<I>(init_args_)...);
    }
...
class ASIO_NODISCARD deferred_conditional
{
  private:
    ...
    explicit deferred_conditional(bool b, T &&on_true, F &&on_false)
        : on_true_(static_cast<T &&>(on_true)), on_false_(static_cast<F &&>(on_false)), bool_(b)
    {
    }
  ...
  public:
    ...
    /// will be executed.
    constexpr explicit deferred_conditional(bool b) : on_true_(), on_false_(), bool_(b)
    {
    }
    ...
    deferred_conditional<T, OnFalse> then(
        T on_true, constraint_t<is_deferred<T>::value> * = 0,
    ...
    deferred_conditional<OnTrue, T> otherwise(
        T on_false, constraint_t<is_deferred<T>::value> * = 0,
        constraint_t<!is_same<conditional_t<true, OnTrue, T>, deferred_noop>::value> * = 0,
...
class deferred_t
{
  public:
    ...
    struct executor_with_default : InnerExecutor
    {
        ...
        executor_with_default(
            ...
            constraint_t<conditional_t<!is_same<InnerExecutor1, executor_with_default>::value,
                                       is_convertible<InnerExecutor1, InnerExecutor>, false_type>::value> = 0) noexcept
            : InnerExecutor(ex)
        {
        }
    };
    ...
    template <typename T>
    constraint_t<is_deferred<decay_t<T>>::value, decay_t<T>> operator()(T &&t) const
    {
