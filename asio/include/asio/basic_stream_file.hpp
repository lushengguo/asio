...
class basic_stream_file : public basic_file<Executor>
{
  ...
  public:
     ...
     */
    explicit basic_stream_file(const executor_type &ex) : basic_file<Executor>(ex)
    {
     ...
     */
    basic_stream_file(const executor_type &ex, const char *path, file_base::flags open_flags) : basic_file<Executor>(ex)
    {
        asio::error_code ec;
        this->impl_.get_service().set_is_stream(this->impl_.get_implementation(), true);
     ...
     */
    template <typename ExecutionContext>
    basic_stream_file(ExecutionContext &context, const char *path, file_base::flags open_flags,
                      constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                   defaulted_constraint> = defaulted_constraint())
        : basic_file<Executor>(context)
    {
     ...
     */
    template <typename ExecutionContext>
    basic_stream_file(ExecutionContext &context, const std::string &path, file_base::flags open_flags,
                      constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                   defaulted_constraint> = defaulted_constraint())
        : basic_file<Executor>(context)
    {
     ...
     */
    template <typename ExecutionContext>
    basic_stream_file(ExecutionContext &context, const native_handle_type &native_file,
                      constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                   defaulted_constraint> = defaulted_constraint())
     ...
     * constructor.
     */
    basic_stream_file(basic_stream_file &&other) noexcept : basic_file<Executor>(std::move(other))
    {
    }
    ...
    {
        return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(initiate_async_write_some(this), token,
                                                                               buffers);
    }
    ...
    auto async_read_some(const MutableBufferSequence &buffers,
        ...
        -> decltype(async_initiate<ReadToken, void(asio::error_code, std::size_t)>(declval<initiate_async_read_some>(),
                                                                                   token, buffers))
    {
        return async_initiate<ReadToken, void(asio::error_code, std::size_t)>(initiate_async_read_some(this), token,
                                                                              buffers);
    }
    ...
    class initiate_async_write_some
    {
      public:
        ...
        void operator()(WriteHandler &&handler, const ConstBufferSequence &buffers) const
        {
            ...
            self_->impl_.get_service().async_write_some(self_->impl_.get_implementation(), buffers, handler2.value,
    ...
    class initiate_async_read_some
    {
      public:
        ...
        void operator()(ReadHandler &&handler, const MutableBufferSequence &buffers) const
        {
            ...
            self_->impl_.get_service().async_read_some(self_->impl_.get_implementation(), buffers, handler2.value,
                                                       self_->impl_.get_executor());
        }
