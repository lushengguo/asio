...
template <typename T>
struct has_immediate_executor_type<T, void_t<typename T::immediate_executor_type>> : true_type
{
};
...
struct default_immediate_executor
{
    ...
    static auto get(const E &e) noexcept -> decltype(asio::require(e, execution::blocking.never))
    {
...
template <typename E>
struct default_immediate_executor<E, enable_if_t<!execution::is_executor<E>::value>, enable_if_t<is_executor<E>::value>>
{
    class type : public E
    {
      public:
        ...
        explicit type(const Executor1 &e,
                      constraint_t<conditional_t<!is_same<Executor1, type>::value, is_convertible<Executor1, E>,
                                                 false_type>::value> = 0) noexcept
            : E(e)
        {
        }

        type(const type &other) noexcept : E(static_cast<const E &>(other))
        {
        }

        type(type &&other) noexcept : E(static_cast<E &&>(other))
        {
        }
...
template <typename T, typename E>
struct associated_immediate_executor_impl<T, E, enable_if_t<!has_immediate_executor_type<T>::value>,
