...
class executor_function
{
  public:
    ...
    executor_function(executor_function &&other) noexcept : impl_(other.impl_)
    {
        other.impl_ = 0;
    }
  ...
  private:
    ...
    template <typename Function, typename Alloc> struct impl : impl_base
    {
        ...
        template <typename F> impl(F &&f, const Alloc &a) : function_(static_cast<F &&>(f)), allocator_(a)
        {
...
class executor_function_view
{
  public:
    template <typename F>
    explicit executor_function_view(F &f) noexcept : complete_(&executor_function_view::complete<F>), function_(&f)
    {
    }
  ...
  private:
    // Helper to complete function invocation.
    template <typename F> static void complete(void *f)
    {
        (*static_cast<F *>(f))();
    }
