...
class io_uring_operation : public operation
{
  ...
  protected:
    ...
    io_uring_operation(const asio::error_code &success_ec, prepare_func_type prepare_func,
                       perform_func_type perform_func, func_type complete_func)
        : operation(complete_func), ec_(success_ec), bytes_transferred_(0), cancellation_key_(0),
          prepare_func_(prepare_func), perform_func_(perform_func)
    {
    }
