...
{
  public:
    io_uring_socket_recv_op_base(const asio::error_code &success_ec, socket_type socket, socket_ops::state_type state,
                                 const MutableBufferSequence &buffers, socket_base::message_flags flags,
                                 func_type complete_func)
        : io_uring_operation(success_ec, &io_uring_socket_recv_op_base::do_prepare,
    ...
    static bool do_perform(io_uring_operation *base, bool after_completion)
    {
        ...
        if ((o->state_ & socket_ops::internal_non_blocking) != 0)
        {
            ...
            if (after_completion || !except_op)
            {
                ...
                {
                    return socket_ops::non_blocking_recv1(
...
class io_uring_socket_recv_op : public io_uring_socket_recv_op_base<MutableBufferSequence>
{
  public:
    ...
    io_uring_socket_recv_op(const asio::error_code &success_ec, int socket, socket_ops::state_type state,
                            const MutableBufferSequence &buffers, socket_base::message_flags flags, Handler &handler,
                            const IoExecutor &io_ex)
        : io_uring_socket_recv_op_base<MutableBufferSequence>(success_ec, socket, state, buffers, flags,
