...
template <typename Payload> class channel_receive : public channel_operation
{
  ...
  protected:
    channel_receive(func_type func) : channel_operation(func)
    {
    }
};
...
class channel_receive_op : public channel_receive<Payload>
{
  public:
    ...
    channel_receive_op(Handler &handler, const IoExecutor &io_ex)
        : channel_receive<Payload>(&channel_receive_op::do_action), handler_(static_cast<Handler &&>(handler)),
    ...
    static void do_action(channel_operation *base, channel_operation::action a, void *v)
    {
        ...
        if (a != channel_operation::destroy_op)
        {
            ...
            asio::detail::completion_payload_handler<Payload, Handler> handler(static_cast<Payload &&>(*payload),
                                                                               o->handler_);
