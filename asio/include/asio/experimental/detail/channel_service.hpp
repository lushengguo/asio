...
class channel_service : public asio::detail::execution_context_service_base<channel_service<Mutex>>
{
  public:
    ...
    {
        // Default constructor.
        base_implementation_type() : receive_state_(block), send_state_(block), max_buffer_size_(0), next_(0), prev_(0)
        {
        }
    ...
    template <typename Traits, typename... Signatures>
    void move_assign(implementation_type<Traits, Signatures...> &impl, channel_service &other_service,
                     implementation_type<Traits, Signatures...> &other_impl);
    ...
    template <typename Message, typename Traits, typename... Signatures, typename... Args>
    std::size_t try_send_n(implementation_type<Traits, Signatures...> &impl, std::size_t count, bool via_dispatch,
                           Args &&...args);
    ...
    void async_send(implementation_type<Traits, Signatures...> &impl,
                    typename implementation_type<Traits, Signatures...>::payload_type &&payload, Handler &handler,
                    const IoExecutor &io_ex)
    {
        ...
        p.p = new (p.v) op(static_cast<typename implementation_type<Traits, Signatures...>::payload_type &&>(payload),
                           handler, io_ex);
  ...
  private:
    ...
    {
        explicit post_receive(channel_receive<Payload> *op) : op_(op)
        {
        }

        template <typename... Args> void operator()(Args &&...args)
        {
            op_->post(asio::detail::completion_message<Signature>(0, static_cast<Args &&>(args)...));
        }
    ...
    {
      public:
        op_cancellation(channel_service *s, implementation_type<Traits, Signatures...> *impl) : service_(s), impl_(impl)
        {
        }
...
struct channel_service<Mutex>::implementation_type : base_implementation_type
{
    ...
    typedef conditional_t<
        has_signature<typename traits_type::receive_cancelled_signature, Signatures...>::value,
        conditional_t<has_signature<typename traits_type::receive_closed_signature, Signatures...>::value,
                      ...
                      asio::detail::completion_payload<Signatures..., typename traits_type::receive_closed_signature>>,
        conditional_t<
            has_signature<typename traits_type::receive_closed_signature, Signatures...,
                          typename traits_type::receive_cancelled_signature>::value,
            asio::detail::completion_payload<Signatures..., typename traits_type::receive_cancelled_signature>,
            asio::detail::completion_payload<Signatures..., typename traits_type::receive_cancelled_signature,
...
struct channel_service<Mutex>::implementation_type<Traits, R()> : channel_service::base_implementation_type
{
    ...
    // Type of an element stored in the buffer.
    typedef conditional_t<
        has_signature<typename traits_type::receive_cancelled_signature, R()>::value,
        conditional_t<has_signature<typename traits_type::receive_closed_signature, R()>::value,
                      asio::detail::completion_payload<R()>,
                      asio::detail::completion_payload<R(), typename traits_type::receive_closed_signature>>,
        conditional_t<has_signature<typename traits_type::receive_closed_signature, R(),
                                    typename traits_type::receive_cancelled_signature>::value,
                      asio::detail::completion_payload<R(), typename traits_type::receive_cancelled_signature>,
                      asio::detail::completion_payload<R(), typename traits_type::receive_cancelled_signature,
                                                       typename traits_type::receive_closed_signature>>>
        payload_type;

    // Construct with empty buffer.
    implementation_type() : buffer_(0)
    {
    }
    ...
    : channel_service::base_implementation_type
{
    ...
    // Type of an element stored in the buffer.
    typedef conditional_t<
        has_signature<typename traits_type::receive_cancelled_signature, R(asio::error_code)>::value,
        conditional_t<
            has_signature<typename traits_type::receive_closed_signature, R(asio::error_code)>::value,
            asio::detail::completion_payload<R(asio::error_code)>,
            asio::detail::completion_payload<R(asio::error_code), typename traits_type::receive_closed_signature>>,
        conditional_t<
            has_signature<typename traits_type::receive_closed_signature, R(asio::error_code),
                          typename traits_type::receive_cancelled_signature>::value,
            asio::detail::completion_payload<R(asio::error_code), typename traits_type::receive_cancelled_signature>,
            asio::detail::completion_payload<R(asio::error_code), typename traits_type::receive_cancelled_signature,
                                             typename traits_type::receive_closed_signature>>>
        payload_type;

    // Construct with empty buffer.
    implementation_type() : size_(0)
    {
        first_.count_ = 0;
    }
  ...
  private:
    ...
    struct value_handler
    {
        ...
        template <typename... Args> void operator()(const asio::error_code &value, Args &&...)
        {
            target_ = value;
        }
    };
