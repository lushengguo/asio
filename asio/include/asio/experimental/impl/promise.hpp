...
template <typename... Ts, typename Executor, typename Allocator> struct promise_impl<void(Ts...), Executor, Allocator>
{
    ...
    template <typename Alloc, typename WaitHandler_> struct completion_impl final : completion_base
    {
        ...
        completion_impl(Alloc_ &&alloc, Handler_ &&wh)
            : handler(std::forward<Handler_>(wh)), allocator(std::forward<Alloc_>(alloc))
        {
        }
    };
    ...
    {
        complete_with_result_impl(asio::detail::make_index_sequence<sizeof...(Ts)>{});
    }
    ...
    template <typename... T_> void cancel_impl_(T_ *...)
    {
        complete(T_{}...);
    }
