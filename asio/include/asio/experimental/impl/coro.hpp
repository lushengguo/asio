                                                  ...
                                                  typename coro_traits<Yield, Return, Executor>::return_type>
{
    ...
    coro_promise(Executor executor, Args &&...args) noexcept
        : coro_promise_allocator<Allocator>(executor, std::forward<Args>(args)...), executor_(std::move(executor))
    {
    }

    template <typename First, typename... Args>
    coro_promise(First &&f, Executor executor, Args &&...args) noexcept
        : coro_promise_allocator<Allocator>(f, executor, std::forward<Args>(args)...), executor_(std::move(executor))
    {
    }
    ...
    coro_promise(First &&f, Context &&ctx, Args &&...args) noexcept
        : coro_promise_allocator<Allocator>(f, ctx, std::forward<Args>(args)...), executor_(ctx.get_executor())
    {
    }

    template <detail::execution_context Context, typename... Args>
    coro_promise(Context &&ctx, Args &&...args) noexcept
        : coro_promise_allocator<Allocator>(ctx, std::forward<Args>(args)...), executor_(ctx.get_executor())
    {
    }
    ...
    // This await transformation resets the associated cancellation state.
    template <typename Filter>
    auto await_transform(this_coro::reset_cancellation_state_1_t<Filter> reset) noexcept
    {
        struct result
        {
            detail::coro_cancellation_source *src_;
            ...
            auto await_resume()
            {
                return src_->reset_cancellation_state(static_cast<Filter &&>(filter_));
            }
        };
    ...
    auto await_transform(this_coro::reset_cancellation_state_2_t<InFilter, OutFilter> reset) noexcept
    {
        struct result
        {
            ...
            auto await_resume()
            {
                return src_->reset_cancellation_state(static_cast<InFilter &&>(in_filter_),
    ...
    // an exception.
    auto await_transform(this_coro::throw_if_cancelled_0_t) noexcept requires(!is_noexcept)
    {
        struct result
        {
            detail::coro_cancellation_source *src_;
    ...
    // exception.
    auto await_transform(this_coro::throw_if_cancelled_1_t throw_if_cancelled) noexcept requires(!is_noexcept)
    {
        struct result
        {
            detail::coro_cancellation_source *src_;
    ...
    auto await_transform(Op &&op, constraint_t<is_async_operation<Op>::value> = 0)
    {
...
struct coro<Yield, Return, Executor, Allocator>::awaitable_t
{
    ...
    auto await_suspend(detail::coroutine_handle<detail::coro_promise<Y, R, E, A>> h) -> detail::coroutine_handle<>
    {
        ...
        {
            coro_.coro_->awaited_from =
                detail::dispatch_coroutine(asio::prefer(hp.get_executor(), execution::outstanding_work.tracked),
                                           [h]() mutable { h.resume(); })
                    .handle;
...
struct coro<Yield, Return, Executor, Allocator>::initiate_async_resume
{
    ...
    explicit initiate_async_resume(coro *self) : coro_(self->coro_)
    {
    }
    ...
    auto handle(E exec, WaitHandler &&handler, std::true_type /* error is noexcept */,
    ...
    template <typename E, typename WaitHandler>
    requires(!std::is_void_v<result_type>) auto handle(E exec, WaitHandler &&handler,
    ...
    template <typename E, typename WaitHandler>
    auto handle(E exec, WaitHandler &&handler, std::false_type /* error is noexcept */,
        ...
        return [the_coro = coro_, h = std::forward<WaitHandler>(handler), exec = std::move(exec)]() mutable {
            if (!the_coro)
                return asio::post(exec,
                                  asio::append(std::move(h), detail::coro_error<error_type>::invalid(), result_type{}));
            ...
            if (!ch)
                ...
                return asio::post(exec,
                                  asio::append(std::move(h), detail::coro_error<error_type>::done(), result_type{}));
            else
            {
    ...
    void operator()(WaitHandler &&handler)
    {
        ...
        asio::dispatch(get_executor(),
                       handle(exec, std::forward<WaitHandler>(handler), std::integral_constant<bool, is_noexcept>{},
