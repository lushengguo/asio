...
{
    typedef asio::detail::array<std::size_t, N> order_type;
    typedef typename parallel_group_signature<N, typename parallel_group_signature<N, Sig0, Sig1>::raw_type,
...
class parallel_group
{
  private:
    struct initiate_async_wait
    {
        ...
        {
            detail::parallel_group_launch(std::forward<Condition>(c), std::forward<Handler>(h), ops,
                                          asio::detail::index_sequence_for<Ops...>());
        }
    };
  ...
  public:
    /// Constructor.
    explicit parallel_group(Ops... ops) : ops_(std::move(ops)...)
    {
    }
    ...
    auto async_wait(CancellationCondition cancellation_condition, CompletionToken &&token)
        -> decltype(asio::async_initiate<CompletionToken, signature>(declval<initiate_async_wait>(), token,
                                                                     std::move(cancellation_condition),
                                                                     std::move(ops_)))
    {
        return asio::async_initiate<CompletionToken, signature>(initiate_async_wait(), token,
 ...
 */
template <typename... Ops>
ASIO_NODISCARD inline parallel_group<Ops...> make_parallel_group(Ops... ops)
{
...
class ranged_parallel_group
{
  ...
  public:
    ...
    explicit ranged_parallel_group(Range range, const Allocator &allocator = Allocator())
        : range_(std::move(range)), allocator_(allocator)
    {
    }

    /// The completion signature for the group of operations.
    typedef typename detail::ranged_parallel_group_signature<
        completion_signature_of_t<decay_t<decltype(*std::declval<typename Range::iterator>())>>, Allocator>::type
        signature;
...
ASIO_NODISCARD inline ranged_parallel_group<decay_t<Range>> make_parallel_group(
    Range &&range, constraint_t<is_async_operation_range<decay_t<Range>>::value> = 0)
{
...
ASIO_NODISCARD inline ranged_parallel_group<decay_t<Range>, Allocator> make_parallel_group(
    allocator_arg_t, const Allocator &allocator, Range &&range,
