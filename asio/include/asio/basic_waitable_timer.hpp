...
template <typename Clock, typename WaitTraits, typename Executor> class basic_waitable_timer
{
  ...
  public:
     ...
     */
    explicit basic_waitable_timer(const executor_type &ex) : impl_(0, ex)
    {
    }
     ...
     * as an absolute time.
     */
    basic_waitable_timer(const executor_type &ex, const time_point &expiry_time) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     */
    template <typename ExecutionContext>
    explicit basic_waitable_timer(ExecutionContext &context, const time_point &expiry_time,
                                  constraint_t<is_convertible<ExecutionContext &, execution_context &>::value> = 0)
        : impl_(0, 0, context)
    {
        asio::error_code ec;
     ...
     * now.
     */
    basic_waitable_timer(const executor_type &ex, const duration &expiry_time) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     */
    template <typename ExecutionContext>
    explicit basic_waitable_timer(ExecutionContext &context, const duration &expiry_time,
                                  constraint_t<is_convertible<ExecutionContext &, execution_context &>::value> = 0)
        : impl_(0, 0, context)
    {
        asio::error_code ec;
