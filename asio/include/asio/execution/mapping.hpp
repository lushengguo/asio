...
struct mapping_t
{
    ...
    constexpr mapping_t() : value_(-1)
    {
    }

    constexpr mapping_t(thread_t) : value_(0)
    {
    }

    constexpr mapping_t(new_thread_t) : value_(1)
    {
    }

    constexpr mapping_t(other_t) : value_(2)
    {
    }
    ...
    template <typename T>
    struct query_member : traits::query_member<typename proxy<T>::type, mapping_t>
    {
    };
    ...
    friend constexpr bool operator==(const mapping_t &a, const mapping_t &b)
    {
    ...
    friend constexpr mapping_t query(const Executor &ex, convertible_from_mapping_t,
...
template <typename T>
struct static_query<
    T, execution::mapping_t::new_thread_t,
