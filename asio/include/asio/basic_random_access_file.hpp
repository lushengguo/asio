...
template <typename Executor> class basic_random_access_file : public basic_file<Executor>
{
  ...
  public:
     ...
     */
    explicit basic_random_access_file(const executor_type &ex) : basic_file<Executor>(ex)
    {
    }
     ...
     */
    template <typename ExecutionContext>
    basic_random_access_file(ExecutionContext &context, const char *path, file_base::flags open_flags,
                             constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                          defaulted_constraint> = defaulted_constraint())
     ...
     */
    template <typename ExecutionContext>
    basic_random_access_file(ExecutionContext &context, const std::string &path, file_base::flags open_flags,
                             constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                          defaulted_constraint> = defaulted_constraint())
        : basic_file<Executor>(context, path, open_flags)
    {
     ...
     */
    template <typename ExecutionContext>
    basic_random_access_file(ExecutionContext &context, const native_handle_type &native_file,
                             constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                          defaulted_constraint> = defaulted_constraint())
     ...
     * constructor.
     */
    basic_random_access_file(basic_random_access_file &&other) noexcept : basic_file<Executor>(std::move(other))
    {
    }
    ...
    {
        return async_initiate<WriteToken, void(asio::error_code, std::size_t)>(initiate_async_write_some_at(this),
    ...
    {
        return async_initiate<ReadToken, void(asio::error_code, std::size_t)>(initiate_async_read_some_at(this), token,
                                                                              offset, buffers);
    }
  ...
  private:
    ...
    class initiate_async_write_some_at
    {
      public:
        ...
        explicit initiate_async_write_some_at(basic_random_access_file *self) : self_(self)
        {
        }
        ...
        void operator()(WriteHandler &&handler, uint64_t offset, const ConstBufferSequence &buffers) const
        {
            ...
            self_->impl_.get_service().async_write_some_at(self_->impl_.get_implementation(), offset, buffers,
    ...
    class initiate_async_read_some_at
    {
      public:
        ...
        explicit initiate_async_read_some_at(basic_random_access_file *self) : self_(self)
        {
        }
        ...
        void operator()(ReadHandler &&handler, uint64_t offset, const MutableBufferSequence &buffers) const
        {
            ...
            self_->impl_.get_service().async_read_some_at(self_->impl_.get_implementation(), offset, buffers,
                                                          handler2.value, self_->impl_.get_executor());
        }
