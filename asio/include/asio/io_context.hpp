...
template <typename Allocator, uintptr_t Bits> class io_context::basic_executor_type : detail::io_context_bits, Allocator
{
  public:
    /// Copy constructor.
    basic_executor_type(const basic_executor_type &other) noexcept
        : Allocator(static_cast<const Allocator &>(other)), target_(other.target_)
    {
    ...
    basic_executor_type(basic_executor_type &&other) noexcept
        : Allocator(static_cast<Allocator &&>(other)), target_(other.target_)
    {
        if (Bits & outstanding_work_tracked)
            other.target_ = 0;
    }
  ...
  private:
    ...
    explicit basic_executor_type(io_context &i) noexcept : Allocator(), target_(reinterpret_cast<uintptr_t>(&i))
    {
        if (Bits & outstanding_work_tracked)
            context_ptr()->impl_.work_started();
    }
    ...
    basic_executor_type(io_context *i, const Allocator &a, uintptr_t bits) noexcept
        : Allocator(a), target_(reinterpret_cast<uintptr_t>(i) | bits)
    {
        if (Bits & outstanding_work_tracked)
            if (context_ptr())
                context_ptr()->impl_.work_started();
...
struct query_static_constexpr_member<
    asio::io_context::basic_executor_type<Allocator, Bits>, Property,
...
template <typename Allocator, uintptr_t Bits, typename Property>
struct query_member<asio::io_context::basic_executor_type<Allocator, Bits>, Property,
