...
class thread_pool::basic_executor_type : detail::thread_pool_bits
{
  public:
    ...
    basic_executor_type(basic_executor_type &&other) noexcept
        : pool_(other.pool_), allocator_(static_cast<Allocator &&>(other.allocator_)), bits_(other.bits_)
    {
        if (Bits & outstanding_work_tracked)
            other.pool_ = 0;
    }
  ...
  private:
    ...
    {
        return basic_executor_type<Allocator, Bits & ~blocking_mask>(pool_, allocator_,
                                                                     (bits_ & ~blocking_mask) | blocking_never);
    }
  ...
  private:
     ...
     */
    template <typename OtherAllocator>
    constexpr Allocator query(execution::allocator_t<OtherAllocator>) const noexcept
    {
        return allocator_;
    }
  ...
  private:
    ...
    explicit basic_executor_type(thread_pool &p) noexcept : pool_(&p), allocator_(), bits_(0)
    {
        if (Bits & outstanding_work_tracked)
            pool_->scheduler_.work_started();
    }
...
template <typename Allocator, unsigned int Bits>
struct require_member<asio::thread_pool::basic_executor_type<Allocator, Bits>, asio::execution::relationship_t::fork_t>
{
    static constexpr bool is_valid = true;
    static constexpr bool is_noexcept = false;
...
template <typename Allocator, unsigned int Bits, typename Property>
struct query_member<asio::thread_pool::basic_executor_type<Allocator, Bits>, Property,
