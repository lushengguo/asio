...
template <typename Protocol, typename Executor> class basic_socket : public socket_base
{
  ...
  public:
     ...
     */
    explicit basic_socket(const executor_type &ex) : impl_(0, ex)
    {
    }
     ...
     */
    basic_socket(const executor_type &ex, const protocol_type &protocol) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     * @throws asio::system_error Thrown on failure.
     */
    basic_socket(const executor_type &ex, const endpoint_type &endpoint) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     */
    template <typename ExecutionContext>
    basic_socket(ExecutionContext &context, const protocol_type &protocol, const native_handle_type &native_socket,
                 constraint_t<is_convertible<ExecutionContext &, execution_context &>::value> = 0)
        : impl_(0, 0, context)
    {
        asio::error_code ec;
     ...
     */
    basic_socket(basic_socket &&other) noexcept : impl_(std::move(other.impl_))
    {
    }
    ...
    auto async_connect(const endpoint_type &peer_endpoint,
        ...
        -> decltype(async_initiate<ConnectToken, void(asio::error_code)>(declval<initiate_async_connect>(), token,
                                                                         peer_endpoint, declval<asio::error_code &>()))
    {
        ...
        return async_initiate<ConnectToken, void(asio::error_code)>(initiate_async_connect(this), token, peer_endpoint,
                                                                    open_ec);
    }
  ...
  private:
    ...
    class initiate_async_connect
    {
      public:
        ...
        void operator()(ConnectHandler &&handler, const endpoint_type &peer_endpoint,
                        const asio::error_code &open_ec) const
        {
            ...
            else
            {
                ...
                self_->impl_.get_service().async_connect(self_->impl_.get_implementation(), peer_endpoint,
