...
class request
{
  public:
        ...
        : version_(version), command_(cmd), user_id_(user_id), null_byte_(0)
    {
        ...
        {
            throw std::system_error(asio::error::address_family_not_supported);
        }
    ...
    {
        return {{asio::buffer(&version_, 1), asio::buffer(&command_, 1), asio::buffer(&port_high_byte_, 1),
                 asio::buffer(&port_low_byte_, 1), asio::buffer(address_), asio::buffer(user_id_),
