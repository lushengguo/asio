...
ASIO_NODISCARD constexpr unspecified reset_cancellation_state(Filter &&filter);
...
{
    template <typename F>
    explicit constexpr reset_cancellation_state_1_t(F &&filt) : filter(static_cast<F &&>(filt))
    {
    }
...
{
    return reset_cancellation_state_2_t<decay_t<InFilter>, decay_t<OutFilter>>(static_cast<InFilter &&>(in_filter),
...
ASIO_NODISCARD inline constexpr throw_if_cancelled_0_t throw_if_cancelled()
{
...
{
    explicit constexpr throw_if_cancelled_1_t(bool val) : value(val)
    {
    }
