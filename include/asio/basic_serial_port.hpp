...
class basic_serial_port : public serial_port_base
{
  ...
  public:
     ...
     * port.
     */
    basic_serial_port(const executor_type &ex, const char *device) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     * port.
     */
    basic_serial_port(const executor_type &ex, const std::string &device) : impl_(0, ex)
    {
        asio::error_code ec;
        impl_.get_service().open(impl_.get_implementation(), device, ec);
        asio::detail::throw_error(ec, "open");
     ...
     */
    basic_serial_port(const executor_type &ex, const native_handle_type &native_serial_port) : impl_(0, ex)
    {
        asio::error_code ec;
     ...
     */
    template <typename ExecutionContext>
    basic_serial_port(ExecutionContext &context, const native_handle_type &native_serial_port,
                      constraint_t<is_convertible<ExecutionContext &, execution_context &>::value> = 0)
        : impl_(0, 0, context)
    {
        asio::error_code ec;
