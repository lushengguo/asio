...
class stream : public stream_base, private noncopyable
{
  ...
  public:
    ...
    stream(Arg &&arg, context &ctx)
        : next_layer_(static_cast<Arg &&>(arg)), core_(ctx.native_handle(), next_layer_.lowest_layer().get_executor())
    {
    }
    ...
    stream(Arg &&arg, native_handle_type handle)
        : next_layer_(static_cast<Arg &&>(arg)), core_(handle, next_layer_.lowest_layer().get_executor())
    {
    }
    ...
    auto async_handshake(handshake_type type, HandshakeToken &&token = default_completion_token_t<executor_type>())
        -> decltype(async_initiate<HandshakeToken, void(asio::error_code)>(declval<initiate_async_handshake>(), token,
                                                                           type))
    {
    ...
    class initiate_async_write_some
    {
      public:
        ...
        void operator()(WriteHandler &&handler, const ConstBufferSequence &buffers) const
        {
            ...
            detail::async_io(self_->next_layer_, self_->core_, detail::write_op<ConstBufferSequence>(buffers),
                             handler2.value);
        }
    ...
    class initiate_async_read_some
    {
      public:
        ...
        void operator()(ReadHandler &&handler, const MutableBufferSequence &buffers) const
        {
            ...
            detail::async_io(self_->next_layer_, self_->core_, detail::read_op<MutableBufferSequence>(buffers),
                             handler2.value);
        }
