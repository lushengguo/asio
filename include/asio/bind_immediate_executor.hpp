...
template <typename T>
struct immediate_executor_binder_argument_type<T, void_t<typename T::argument_type>>
{
...
template <typename T>
struct immediate_executor_binder_argument_types<T, void_t<typename T::first_argument_type>>
{
...
#endif // !defined(GENERATING_DOCUMENTATION)
{
  public:
     ...
     */
    template <typename U>
    immediate_executor_binder(const immediate_executor_type &e, U &&u) : executor_(e), target_(static_cast<U &&>(u))
    {
    }
    ...
    /// specify a different immediate executor.
    template <typename U, typename OtherExecutor>
    immediate_executor_binder(const immediate_executor_type &e, immediate_executor_binder<U, OtherExecutor> &&other,
                              constraint_t<is_constructible<T, U>::value> = 0)
...
struct partial_immediate_executor_binder
{
    ...
    explicit partial_immediate_executor_binder(const Executor &ex) : executor_(ex)
    {
    }
    ...
    ASIO_NODISCARD inline constexpr immediate_executor_binder<decay_t<CompletionToken>, Executor> operator()(
...
template <typename Executor>
ASIO_NODISCARD inline partial_immediate_executor_binder<Executor> bind_immediate_executor(const Executor &ex)
{
...
class immediate_executor_binder_completion_handler_async_result<
    TargetAsyncResult, Executor, void_t<typename TargetAsyncResult::completion_handler_type>>
{
  private:
    ...
    struct init_wrapper : detail::initiation_base<Initiation>
    {
        ...
        void operator()(Handler &&handler, const Executor &e, Args &&...args) &&
        {
            static_cast<Initiation &&> (*this)(
                immediate_executor_binder<decay_t<Handler>, Executor>(e, static_cast<Handler &&>(handler)),
    ...
    static auto initiate(Initiation &&initiation, RawCompletionToken &&token, Args &&...args)
        -> decltype(async_initiate<conditional_t<is_const<remove_reference_t<RawCompletionToken>>::value, const T, T>,
                                   Signature>(declval<init_wrapper<decay_t<Initiation>>>(), token.get(),
                                              token.get_immediate_executor(), static_cast<Args &&>(args)...))
    {
        return async_initiate<conditional_t<is_const<remove_reference_t<RawCompletionToken>>::value, const T, T>,
                              Signature>(init_wrapper<decay_t<Initiation>>(static_cast<Initiation &&>(initiation)),
                                         token.get(), token.get_immediate_executor(), static_cast<Args &&>(args)...);
    }
...
struct async_result<partial_immediate_executor_binder<Executor>, Signatures...>
{
    ...
    static auto initiate(Initiation &&initiation, RawCompletionToken &&token, Args &&...args)
        -> decltype(async_initiate<Signatures...>(
            ...
            immediate_executor_binder<default_completion_token_t<associated_executor_t<Initiation>>, Executor>(
                token.executor_, default_completion_token_t<associated_executor_t<Initiation>>{}),
            static_cast<Args &&>(args)...))
    {
