...
template <typename T>
struct cancellation_slot_binder_argument_type<T, void_t<typename T::argument_type>>
{
...
template <typename T>
struct cancellation_slot_binder_argument_types<T, void_t<typename T::first_argument_type>>
{
...
#endif // !defined(GENERATING_DOCUMENTATION)
{
  public:
     ...
     */
    template <typename U>
    cancellation_slot_binder(const cancellation_slot_type &s, U &&u) : slot_(s), target_(static_cast<U &&>(u))
    {
    }

    /// Copy constructor.
    cancellation_slot_binder(const cancellation_slot_binder &other)
        : slot_(other.get_cancellation_slot()), target_(other.get())
    {
    }
    ...
    cancellation_slot_binder(const cancellation_slot_type &s, cancellation_slot_binder &&other)
        : slot_(s), target_(static_cast<T &&>(other.get()))
    {
    }
    ...
    cancellation_slot_binder(const cancellation_slot_type &s,
                             ...
                             constraint_t<is_constructible<T, U>::value> = 0)
        : slot_(s), target_(static_cast<U &&>(other.get()))
    {
    }
...
struct partial_cancellation_slot_binder
{
    ...
    explicit partial_cancellation_slot_binder(const CancellationSlot &ex) : cancellation_slot_(ex)
    {
    }
    ...
    ASIO_NODISCARD inline constexpr cancellation_slot_binder<decay_t<CompletionToken>, CancellationSlot> operator()(
...
template <typename CancellationSlot>
ASIO_NODISCARD inline partial_cancellation_slot_binder<CancellationSlot> bind_cancellation_slot(
...
ASIO_NODISCARD inline cancellation_slot_binder<decay_t<T>, CancellationSlot> bind_cancellation_slot(
    const CancellationSlot &s, T &&t)
{
...
class cancellation_slot_binder_completion_handler_async_result<
    TargetAsyncResult, CancellationSlot, void_t<typename TargetAsyncResult::completion_handler_type>>
{
  private:
      ...
      public detail::cancellation_slot_binder_async_result_return_type<async_result<T, Signature>>
{
  public:
    ...
    struct init_wrapper : detail::initiation_base<Initiation>
    {
        ...
        void operator()(Handler &&handler, const CancellationSlot &slot, Args &&...args) &&
        {
            static_cast<Initiation &&> (*this)(
                cancellation_slot_binder<decay_t<Handler>, CancellationSlot>(slot, static_cast<Handler &&>(handler)),
                static_cast<Args &&>(args)...);
        }

        template <typename Handler, typename... Args>
        void operator()(Handler &&handler, const CancellationSlot &slot, Args &&...args) const &
        {
    ...
    static auto initiate(Initiation &&initiation, RawCompletionToken &&token, Args &&...args)
        -> decltype(async_initiate<conditional_t<is_const<remove_reference_t<RawCompletionToken>>::value, const T, T>,
                                   Signature>(declval<init_wrapper<decay_t<Initiation>>>(), token.get(),
                                              token.get_cancellation_slot(), static_cast<Args &&>(args)...))
    {
        return async_initiate<conditional_t<is_const<remove_reference_t<RawCompletionToken>>::value, const T, T>,
                              Signature>(init_wrapper<decay_t<Initiation>>(static_cast<Initiation &&>(initiation)),
                                         token.get(), token.get_cancellation_slot(), static_cast<Args &&>(args)...);
    }
...
struct async_result<partial_cancellation_slot_binder<CancellationSlot>, Signatures...>
{
    template <typename Initiation, typename RawCompletionToken, typename... Args>
    static auto initiate(Initiation &&initiation, RawCompletionToken &&token, Args &&...args)
        -> decltype(async_initiate<Signatures...>(
            static_cast<Initiation &&>(initiation),
            cancellation_slot_binder<default_completion_token_t<associated_executor_t<Initiation>>, CancellationSlot>(
                token.cancellation_slot_, default_completion_token_t<associated_executor_t<Initiation>>{}),
            static_cast<Args &&>(args)...))
    {
...
template <template <typename, typename> class Associator, typename T, typename CancellationSlot,
