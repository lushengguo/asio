...
class strand
{
  public:
     ...
     * constructible.
     */
    strand() : executor_(), impl_(strand::create_implementation(executor_))
    {
    }
    ...
    explicit strand(const Executor1 &e,
                    constraint_t<conditional_t<!is_same<Executor1, strand>::value, is_convertible<Executor1, Executor>,
                                               false_type>::value> = 0)
        : executor_(e), impl_(strand::create_implementation(executor_))
    {
    }
    ...
    strand(const strand<OtherExecutor> &other) noexcept : executor_(other.executor_), impl_(other.impl_)
    {
    }
    ...
    strand(strand &&other) noexcept
        : executor_(static_cast<Executor &&>(other.executor_)), impl_(static_cast<implementation_type &&>(other.impl_))
    {
    }
    ...
    constraint_t<can_query<const Executor &, Property>::value,
                 conditional_t<is_convertible<Property, execution::blocking_t>::value, execution::blocking_t,
  ...
  private:
    ...
    static implementation_type create_implementation(
        const InnerExecutor &ex, constraint_t<can_query<InnerExecutor, execution::context_t>::value> = 0)
    {
    ...
    strand(const Executor &ex, const implementation_type &impl) : executor_(ex), impl_(impl)
    {
    }
 ...
 */
template <typename Executor>
inline strand<Executor> make_strand(
...
struct query_member<strand<Executor>, Property, enable_if_t<can_query<const Executor &, Property>::value>>
{
    ...
    typedef conditional_t<is_convertible<Property, execution::blocking_t>::value, execution::blocking_t,
