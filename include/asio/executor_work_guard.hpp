...
template <typename Executor>
class executor_work_guard<Executor, enable_if_t<is_executor<Executor>::value>>
{
  public:
    typedef Executor executor_type;

    explicit executor_work_guard(const executor_type &e) noexcept : executor_(e), owns_(true)
    {
        executor_.on_work_started();
    }

    executor_work_guard(const executor_work_guard &other) noexcept : executor_(other.executor_), owns_(other.owns_)
    {
        if (owns_)
            executor_.on_work_started();
    }
    ...
    executor_work_guard(executor_work_guard &&other) noexcept
        : executor_(static_cast<Executor &&>(other.executor_)), owns_(other.owns_)
    {
        other.owns_ = false;
    }
 ...
 */
template <typename Executor>
ASIO_NODISCARD inline executor_work_guard<Executor> make_work_guard(
...
ASIO_NODISCARD inline executor_work_guard<typename ExecutionContext::executor_type> make_work_guard(
...
ASIO_NODISCARD inline executor_work_guard<associated_executor_t<T, Executor>> make_work_guard(
...
make_work_guard(const T &t, ExecutionContext &ctx, constraint_t<!is_executor<T>::value> = 0,
