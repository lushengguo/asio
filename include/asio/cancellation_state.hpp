...
struct cancellation_filter
{
    ...
    cancellation_type_t operator()(cancellation_type_t type) const noexcept
    {
        return type & Mask;
    }
};
...
class cancellation_state
{
  public:
    ...
    constexpr cancellation_state() noexcept : impl_(0)
    {
    }
    ...
    constexpr cancellation_state(CancellationSlot slot, InFilter in_filter, OutFilter out_filter)
        : impl_(slot.is_connected() ? &slot.template emplace<impl<InFilter, OutFilter>>(
  ...
  private:
    ...
    template <typename InFilter = enable_terminal_cancellation, typename OutFilter = InFilter>
    struct impl : impl_base
    {
        impl() : in_filter_(), out_filter_()
        {
        }

        impl(InFilter in_filter, OutFilter out_filter)
            : in_filter_(static_cast<InFilter &&>(in_filter)), out_filter_(static_cast<OutFilter &&>(out_filter))
        {
        }
