...
class basic_concurrent_channel
    ...
    : public detail::channel_send_functions<basic_concurrent_channel<Executor, Traits, Signatures...>, Executor,
                                            Signatures...>
#endif // !defined(GENERATING_DOCUMENTATION)
{
  private:
    ...
    auto do_async_receive(asio::detail::completion_payload<PayloadSignatures...> *, CompletionToken &&token)
  ...
  public:
    ...
    basic_concurrent_channel(const executor_type &ex, std::size_t max_buffer_size = 0)
        : service_(&asio::use_service<service_type>(basic_concurrent_channel::get_context(ex))), impl_(), executor_(ex)
    {
    ...
    basic_concurrent_channel(ExecutionContext &context, std::size_t max_buffer_size = 0,
                             constraint_t<is_convertible<ExecutionContext &, execution_context &>::value,
                                          defaulted_constraint> = defaulted_constraint())
        : service_(&asio::use_service<service_type>(context)), impl_(), executor_(context.get_executor())
    {
        service_->construct(impl_, max_buffer_size);
    }
     ...
     * constructor.
     */
    basic_concurrent_channel(basic_concurrent_channel &&other) : service_(other.service_), executor_(other.executor_)
    {
    ...
    template <typename... Args, ASIO_COMPLETION_TOKEN_FOR(void(asio::error_code))
  ...
  private:
    ...
    class initiate_async_send
    {
      public:
        ...
        explicit initiate_async_send(basic_concurrent_channel *self) : self_(self)
        {
        }
        ...
        void operator()(SendHandler &&handler, payload_type &&payload) const
        {
            ...
            self_->service_->async_send(self_->impl_, static_cast<payload_type &&>(payload), handler2.value,
                                        self_->get_executor());
        }
    ...
    class initiate_async_receive
    {
      public:
        ...
        explicit initiate_async_receive(basic_concurrent_channel *self) : self_(self)
        {
        }
