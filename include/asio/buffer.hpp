...
class const_buffer
{
  public:
    /// Construct an empty buffer.
    const_buffer() noexcept : data_(0), size_(0)
    {
    }
...
class const_buffers_1 : public const_buffer
{
  public:
    ...
    /// Construct to represent a given memory range.
    const_buffers_1(const void *data, std::size_t size) noexcept : const_buffer(data, size)
    {
    }
    ...
    explicit const_buffers_1(const const_buffer &b) noexcept : const_buffer(b)
    {
    }
...
inline const const_buffer *buffer_sequence_begin(
    const ConstBuffer &b, constraint_t<is_convertible<const ConstBuffer *, const const_buffer *>::value> = 0) noexcept
{
...
inline auto buffer_sequence_begin(C &c,
                                  constraint_t<!is_convertible<const C *, const mutable_buffer *>::value &&
                                               !is_convertible<const C *, const const_buffer *>::value> = 0) noexcept
    -> decltype(c.begin())
{
    return c.begin();
}
...
inline std::size_t buffer_size(one_buffer, Iterator begin, Iterator) noexcept
{
    return const_buffer(*begin).size();
}

template <typename Iterator>
inline std::size_t buffer_size(multiple_buffers, Iterator begin, Iterator end) noexcept
{
 ...
 */
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(const mutable_buffer &b) noexcept
{
...
ASIO_NODISCARD inline ASIO_MUTABLE_BUFFER buffer(PodType (&data)[N]) noexcept
{
...
template <typename TargetIterator, typename SourceIterator>
inline std::size_t buffer_copy(one_buffer, one_buffer, TargetIterator target_begin, TargetIterator,
                               SourceIterator source_begin, SourceIterator, std::size_t max_bytes_to_copy) noexcept
{
...
template <typename TargetIterator, typename SourceIterator>
std::size_t buffer_copy(multiple_buffers, one_buffer, TargetIterator target_begin, TargetIterator target_end,
...
template <typename TargetIterator, typename SourceIterator>
std::size_t buffer_copy(multiple_buffers, multiple_buffers, TargetIterator target_begin, TargetIterator target_end,
...
template <typename TargetIterator, typename SourceIterator>
std::size_t buffer_copy(multiple_buffers, multiple_buffers, TargetIterator target_begin, TargetIterator target_end,
                        SourceIterator source_begin, SourceIterator source_end, std::size_t max_bytes_to_copy) noexcept
{
    std::size_t total_bytes_copied = 0;
 ...
 */
template <typename MutableBufferSequence, typename ConstBufferSequence>
inline std::size_t buffer_copy(const MutableBufferSequence &target, const ConstBufferSequence &source,
...
template <unsigned char... Bytes>
const unsigned char bin_literal<bytes<Bytes...>>::data[sizeof...(Bytes)] = {Bytes...};

template <unsigned char... Bytes, char Bit7, char Bit6, char Bit5, char Bit4, char Bit3, char Bit2, char Bit1,
          char Bit0, char... Chars>
struct bin_literal<bytes<Bytes...>, Bit7, Bit6, Bit5, Bit4, Bit3, Bit2, Bit1, Bit0, Chars...>
    : bin_literal<bytes<Bytes..., static_cast<unsigned char>((Bit7 == '1' ? 0x80 : 0) | (Bit6 == '1' ? 0x40 : 0) |
...
template <unsigned char... Bytes>
const unsigned char hex_literal<bytes<Bytes...>>::data[sizeof...(Bytes)] = {Bytes...};
...
template <template <typename, char...> class Literal, typename Clean, char... Raw>
...
template <char... Chars>
struct literal<'0', 'b', Chars...> : remove_separators<bin_literal, chars<>, Chars...>
{
};
...
template <char... Chars>
struct literal<'0', 'x', Chars...> : remove_separators<hex_literal, chars<>, Chars...>
{
};
